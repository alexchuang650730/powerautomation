...
class GeneralAgentFeatures:
    ...
    def get_feature_by_name(self, feature_type, feature_name):
        """根据名称获取特定特性"""
        feature_map = {
            "platform": self.platform_features,
            "ui_layout": self.ui_layout_features,
            "prompt": self.prompt_features,
            "thinking": self.thinking_features,
            "content": self.content_features,
            "memory": self.memory_features
        }
        
        if feature_type not in feature_map:
            raise ValueError(f"未知特性类型: {feature_type}")
        
        features = feature_map[feature_type]
        if feature_name not in features:
            raise ValueError(f"在 {feature_type} 中未找到特性: {feature_name}")
        
        return features[feature_name]
    
    def is_feature_enabled(self, feature_type, feature_name):
        """检查特定特性是否启用"""
        feature = self.get_feature_by_name(feature_type, feature_name)
        return feature.get("enabled", False)
    
    def get_feature_config(self, feature_type, feature_name):
        """获取特定特性的配置"""
        feature = self.get_feature_by_name(feature_type, feature_name)
    ...
    def get_core_capabilities(self):
        """获取通用智能体四大核心能力"""
        return {
            "automated_testing": self.platform_features["automated_testing"],
            "agent_manufacturing": self.thinking_features["agent_manufacturing"],
            "workflow_visualization": self.ui_layout_features["n8n_style_workflow_visualization"],
            "ui_layout_automation": {
                "name": "自动化UI布局",
                "description": "自适应不同屏幕尺寸，确保在桌面和移动设备上的良好体验，支持主题定制",
                "enabled": True,
                "config": {
                    "responsive_design": self.ui_layout_features["responsive_design"]["config"],
                    "theme_customization": self.ui_layout_features["theme_customization"]["config"],
