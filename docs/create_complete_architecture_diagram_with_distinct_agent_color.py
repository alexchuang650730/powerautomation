...
# 定义颜色
colors = {
    'central': (93, 173, 226),  # 中央协调器 - 蓝色
    'planner': (93, 173, 226),  # 规划器 - 蓝色
    'brainstorm': (72, 201, 176),  # 头脑风暴器 - 青色
    'recorder': (236, 112, 99),  # 记录器 - 红色
    'release': (245, 176, 65),  # Release管理器 - 橙色
    'collector': (230, 126, 34),  # 测试收集器 - 深橙色
    'solver': (175, 122, 197),  # 问题解决器 - 紫色
    'external': (171, 178, 185),  # 外部系统 - 灰色
    'submodule': (171, 178, 185),  # 子模块 - 灰色
    'agent': (46, 134, 193),  # 智能体 - 深蓝色，与MCP协调器明显区分
    'tool': (76, 175, 80),  # 开发工具 - 绿色
}
...
nodes = {
    ...
    # 外部系统 - 调整mcp.so位置，避免遮挡
    'mcp_so': {'pos': (60, 150), 'width': 120, 'height': 50, 'color': colors['external'], 'label': 'mcp.so', 'font': font_label},
    'github': {'pos': (width - 100, 150), 'width': 120, 'height': 50, 'color': colors['external'], 'label': 'GitHub', 'font': font_label},
    'github_actions': {'pos': (width - 100, 250), 'width': 120, 'height': 50, 'color': colors['external'], 'label': 'GitHub Actions', 'font': font_label},
    ...
    # 新增无限上下文适配器
    'infinite_context': {'pos': (width/2 - 450, 520), 'width': 180, 'height': 30, 'color': colors['submodule'], 'label': 'InfiniteContextAdapter', 'font': font_submodule},
    
    # 新增mcp.so适配器
    'mcp_so_adapter': {'pos': (width/2 - 150, 370), 'width': 150, 'height': 30, 'color': colors['submodule'], 'label': 'MCPSoAdapter', 'font': font_submodule},
}
...
connections = [
    # 智能体连接 - 修正为智能体派发任务
    {'from': 'ppt_agent', 'to': 'central', 'color': (0, 128, 0), 'label': '任务派发'},
    {'from': 'web_agent', 'to': 'central', 'color': (0, 128, 0), 'label': '任务派发'},
    {'from': 'code_agent', 'to': 'central', 'color': (0, 128, 0), 'label': '任务派发'},
    {'from': 'general_agent', 'to': 'central', 'color': (0, 128, 0), 'label': '任务派发'},
    ...
    # mcp.so连接 - 修正为工具调用
    {'from': 'planner', 'to': 'mcp_so', 'color': (128, 128, 128), 'label': '工具调用'},
    {'from': 'central', 'to': 'github', 'color': (128, 128, 128), 'label': '同步'},
    
    # GitHub Actions连接 - 新增与Release Manager的连接
    {'from': 'release', 'to': 'github_actions', 'color': (128, 128, 128), 'label': '触发CI/CD'},
    {'from': 'github_actions', 'to': 'release', 'color': (128, 128, 128), 'label': '构建状态反馈'},
    {'from': 'github_actions', 'to': 'github', 'color': (128, 128, 128), 'label': '代码部署'},
    ...
    {'from': 'planner', 'to': 'mcp_so_adapter', 'color': (0, 128, 0), 'label': '调用'},
    ...
    {'from': 'recorder', 'to': 'infinite_context', 'color': (0, 0, 255), 'label': '调用'},
