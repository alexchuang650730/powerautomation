...
try:
    ...
    font_submodule = ImageFont.truetype("DejaVuSans.ttf", 10)  # 子模块使用更小的字体
except IOError:
    # 尝试其他常见字体
    try:
        ...
        font_submodule = ImageFont.truetype("Arial.ttf", 10)
    except IOError:
...
nodes = {
    # 中央协调器
    'central': {'pos': (600, 150), 'width': 400, 'height': 80, 'color': colors['central'], 'label': 'MCP中央协调器\n(MCPCentralCoordinator)', 'font': font_label},
    
    # 规划器和头脑风暴器
    'planner': {'pos': (300, 300), 'width': 250, 'height': 70, 'color': colors['planner'], 'label': 'MCP规划器\n(MCPPlanner)', 'font': font_label},
    'brainstorm': {'pos': (900, 300), 'width': 250, 'height': 70, 'color': colors['brainstorm'], 'label': 'MCP头脑风暴器\n(MCPBrainstorm)', 'font': font_label},
    
    # 工具模块
    'recorder': {'pos': (150, 450), 'width': 220, 'height': 70, 'color': colors['recorder'], 'label': '思考与操作记录器\n(ThoughtActionRecorder)', 'font': font_label},
    'release': {'pos': (400, 450), 'width': 220, 'height': 70, 'color': colors['release'], 'label': 'Release管理器\n(ReleaseManager)', 'font': font_label},
    'collector': {'pos': (650, 450), 'width': 220, 'height': 70, 'color': colors['collector'], 'label': '测试与问题收集器\n(TestAndIssueCollector)', 'font': font_label},
    'solver': {'pos': (900, 450), 'width': 220, 'height': 70, 'color': colors['solver'], 'label': 'Agent问题解决驱动器\n(AgentProblemSolver)', 'font': font_label},
    
    # 外部系统
    'mcp_so': {'pos': (100, 150), 'width': 120, 'height': 50, 'color': colors['external'], 'label': 'mcp.so', 'font': font_label},
    'github': {'pos': (1100, 150), 'width': 120, 'height': 50, 'color': colors['external'], 'label': 'GitHub', 'font': font_label},
    
    # 子模块 - 规划器 (确保足够宽以容纳文本)
    'matcher': {'pos': (200, 370), 'width': 120, 'height': 30, 'color': colors['submodule'], 'label': 'MCPMatcher', 'font': font_submodule},
    'executor': {'pos': (350, 370), 'width': 120, 'height': 30, 'color': colors['submodule'], 'label': 'MCPExecutor', 'font': font_submodule},
    'cache': {'pos': (200, 400), 'width': 150, 'height': 30, 'color': colors['submodule'], 'label': 'MCPCacheManager', 'font': font_submodule},
    'currency': {'pos': (350, 400), 'width': 150, 'height': 30, 'color': colors['submodule'], 'label': 'CurrencyController', 'font': font_submodule},
    
    # 子模块 - 头脑风暴器 (确保足够宽以容纳文本)
    'analyzer': {'pos': (800, 370), 'width': 150, 'height': 30, 'color': colors['submodule'], 'label': 'CapabilityAnalyzer', 'font': font_submodule},
    'converter': {'pos': (980, 370), 'width': 150, 'height': 30, 'color': colors['submodule'], 'label': 'MCPConverter', 'font': font_submodule},
    'enhancer': {'pos': (900, 400), 'width': 150, 'height': 30, 'color': colors['submodule'], 'label': 'SearchEnhancer', 'font': font_submodule},
    
    # 子模块 - 记录器 (确保足够宽以容纳文本)
    'visual': {'pos': (100, 550), 'width': 180, 'height': 30, 'color': colors['submodule'], 'label': 'VisualThoughtRecorder', 'font': font_submodule},
    'enhanced': {'pos': (300, 550), 'width': 180, 'height': 30, 'color': colors['submodule'], 'label': 'EnhancedThoughtRecorder', 'font': font_submodule},
    
    # 子模块 - Release管理器 (确保足够宽以容纳文本)
    'rules': {'pos': (400, 550), 'width': 180, 'height': 30, 'color': colors['submodule'], 'label': 'ReleaseRulesChecker', 'font': font_submodule},
    
    # 子模块 - 测试收集器 (确保足够宽以容纳文本)
    'updater': {'pos': (650, 550), 'width': 180, 'height': 30, 'color': colors['submodule'], 'label': 'TestReadmeUpdater', 'font': font_submodule},
    
    # 子模块 - 问题解决器 (确保足够宽以容纳文本)
    'manager': {'pos': (850, 550), 'width': 180, 'height': 30, 'color': colors['submodule'], 'label': 'SessionSavePointManager', 'font': font_submodule},
    'rollback': {'pos': (1050, 550), 'width': 150, 'height': 30, 'color': colors['submodule'], 'label': 'RollbackExecutor', 'font': font_submodule},
}
...
# 保存图像
output_path = '/home/ubuntu/powerautomation_integration/docs/images/mcp_architecture_updated.png'
image.save(output_path)

print(f"架构图已生成并保存到 {output_path}")
