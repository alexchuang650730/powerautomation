...
for i, layer in enumerate(layers):
    ...
    # 绘制层名称 - 添加白色背景确保可读性
    text_bg = patches.Rectangle((layer_x + 0.1, y - 0.45), len(layer["name"])*0.18, 0.3, 
                              facecolor=colors["background"], alpha=0.7, edgecolor=None)
    ...
    for j, component in enumerate(layer["components"]):
        # 为每个组件文本添加白色背景
        text_bg = patches.Rectangle((layer_x + 0.1, y - 0.75 - j * 0.3), len(component)*0.12, 0.25, 
                                  facecolor=colors["background"], alpha=0.7, edgecolor=None)
        ax.add_patch(text_bg)
...
for i in range(len(layers) - 1):
    ...
    # 绘制标签背景 - 增加背景尺寸
    text_bg = patches.Rectangle((mid_x - 0.6, (start_y + end_y) / 2 - 0.15), 1.2, 0.3, 
...
# 添加横向功能分组 - 调整位置和大小避免遮挡
feature_start_x = 0.5
feature_width = 4  # 增加宽度
feature_height = 5  # 增加高度
feature_y = 7  # 降低位置，避免与主层次结构重叠
...
# 为标题添加白色背景
title_bg = patches.Rectangle((feature_start_x + 0.1, feature_y - 0.45), 3.8, 0.3, 
                          facecolor=colors["background"], alpha=0.7, edgecolor=None)
...
for i, line in enumerate(existing_features):
    # 为每行文本添加白色背景
    if line:  # 跳过空行
        text_bg = patches.Rectangle((feature_start_x + 0.1, feature_y - 0.75 - i * 0.35), len(line)*0.12, 0.25, 
                                  facecolor=colors["background"], alpha=0.7, edgecolor=None)
        ax.add_patch(text_bg)
    
    ax.text(feature_start_x + 0.2, feature_y - 0.6 - i * 0.35,  # 增加行间距
            line, fontsize=10)
...
# 为标题添加白色背景
title_bg = patches.Rectangle((future_start_x + 0.1, feature_y - 0.45), 3.8, 0.3, 
                          facecolor=colors["background"], alpha=0.7, edgecolor=None)
ax.add_patch(title_bg)
...
for i, line in enumerate(upcoming_features):
    # 为每行文本添加白色背景
    if line:  # 跳过空行
        text_bg = patches.Rectangle((future_start_x + 0.1, feature_y - 0.75 - i * 0.3), len(line)*0.12, 0.25, 
                                  facecolor=colors["background"], alpha=0.7, edgecolor=None)
        ax.add_patch(text_bg)
    
    ax.text(future_start_x + 0.2, feature_y - 0.6 - i * 0.3, 
            line, fontsize=10)

# 设置坐标轴
ax.set_xlim(0, 18)  # 增加宽度
ax.set_ylim(1, 18)  # 增加高度范围
ax.axis('off')

# 保存图像
output_path = "images/powerautomation_layered_architecture_improved.png"
